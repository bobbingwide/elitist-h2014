	...
	foreach ( $options as $value ) {
		...
		case "radio":
			...
			foreach ($value['options'] as $key => $option) {
				...
				if ($value['custom'] == "cufon") { 
					$output .= '<input class="of-input of-radio" type="radio" name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '" value="'. esc_attr( $key ) . '" '. checked( $val, $key, false) .' /><span id="cufon-'.$cufon_i.'" style="font-size:22px;" >' . esc_html( $option ) . '</span>';
					$output .= '<br /><br />';
				}else{
		...
		case "cufon":
			$name = $option_name .'['. $value['id'] .']';
			 ...
			 { 
				...
				$output .= '<label id="cufon-'.$cufon_i.'" style="font-size:22px;" for="' . esc_attr( $id ) . '">' . esc_html( $option ) . '</label>';
				$output .= '<br /><br />';
				$cufon_i++;
			}
			break;
		...
		case "sidebar":
			...
			<table class="widefat page" id="sbg_table">
				...
				if(is_array($sidebars) && !empty($sidebars)){
					...
					$slug_sidebar = name_to_class($sidebar);
				
                	$numero = $cnt+1;
					$output .= "<tr><td>".$sidebar."</td><td>".$slug_sidebar."</td><td><a href='javascript:void(0);' onclick='return remove_sidebar_link(\"".$sidebar."\",".$numero.");' title='Remove this sidebar'>remove</a></td></tr>";
		...
		case "portfolio":
			...
			$output .= '<div class="add_portfolio">
				<a href="javascript:void(0);" onclick="return add_portfolio_link()" title="Add a portfolio">+ Add portfolio</a>
			</div>
			<br />
			<table class="widefat page" id="sbg_table_portfolio">
				<tr>
					<th>Name</th>
					<th>Slug</th>
					<th>Remove</th>
				...
				if(is_array($portfolios) && !empty($portfolios)){
					...
					$slug_portfolio = name_to_class($portfolio);
				
                	$numero = $cnt+1;
					$output .= "<tr><td>".$portfolio."</td><td>".$slug_portfolio."</td><td><a href='javascript:void(0);' onclick='return remove_portfolio_link(\"".$portfolio."\",".$numero.");' title='Remove this portfolio'>remove</a></td></tr>";
					...
					$output .= "<tr>
						<td colspan='3'>No portfolios defined</td>
					</tr>";
